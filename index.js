...
function getYears(arr, cb) {
  /* code here */
  return cb(arr).map((team) => team.Year);
}
...
function getWinnersByYear(arr, finalsCb, yearsCb, winnersCb) {
  /* code here */
  const teams = finalsCb(arr);
  const years = yearsCb(arr, finalsCb);
  const winners = winnersCb(arr, finalsCb);

  return teams.map((team, index) => `In ${years[index]}, ${winners[index]} won the world cup!`);
}
...
function getAverageGoals(data) {
  ...
  return (
    data.reduce((prev, next) => {
      return prev + next["Home Team Goals"] + next["Away Team Goals"];
    }, 0) / data.length
  ).toFixed(2);
}
